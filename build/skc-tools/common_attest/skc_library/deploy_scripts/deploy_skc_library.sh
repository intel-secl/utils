...
install_psw_qgl()
{
	...
	sed -i "s|PCCS_URL=.*|PCCS_URL=https://$CSP_TCS_IP:$CSP_TCS_PORT/tcs/v4/sgx/|g" /etc/sgx_default_qcnl.conf
	sed -i "s|.*USE_SECURE_CERT=.*|USE_SECURE_CERT=FALSE|g" /etc/sgx_default_qcnl.conf
	
	#Update TCS root CA Certificate in SGX Compute node certificate store in order for  QPL to verify TCS
