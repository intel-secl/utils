...
install_prereqs()
{
		...
		$PKGMGR install -qy https://dl.fedoraproject.org/pub/epel/epel-release-latest-8.noarch.rpm || exit 1
                $PKGMGR install -y http://rpmfind.net/linux/fedora/linux/releases/32/Everything/x86_64/os/Packages/m/msr-tools-1.3-13.fc32.x86_64.rpm || exit 1
...
uninstall()
{
        ...
        if [ "$OS" == "rhel" ]; then
                $PKGMGR remove -y libsgx-ra-uefi libsgx-launch libsgx-uae-service libsgx-urts libsgx-ae-qve libsgx-dcap-ql libsgx-dcap-ql-devel libsgx-dcap-default-qpl-devel libsgx-dcap-default-qpl
        elif [ "$OS" == "ubuntu" ]; then
...
install_multipackage_agent_rpm()
{
		...
		rpm -ivh libsgx-ra-uefi-$MP_RPM_VER.el8.x86_64.rpm
		popd
                rm -rf sgx_rpm_local_repo sgx_rpm_local_repo.tgz
        elif [ "$OS" == "ubuntu" ]; then
