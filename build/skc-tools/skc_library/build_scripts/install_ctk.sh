...
install_cryptoapitoolkit()
{
	...
	mkdir -p $GIT_CLONE_PATH
        ...
        ./configure --with-p11-kit-path=$P11_KIT_PATH --prefix=$CTK_INSTALL --enable-dcap --enable-ephemeral-quote --disable-multiprocess-support --enable-session-key-wrap || exit 1
	make install || exit 1
	popd
